...
class EnemySystem {
    ...
    generatePatrolPoints() {
        for(let i=0; i<8; i++) {
            ...
            do {
                ...
                attempts++;
            } while((mapGrid[gy][gx] === 1 || attempts > 100) && attempts < 100);
    ...
    update(delta, time) {
        ...
        if (this.state === 'PATROL' || this.state === 'LOST') {
            ...
            else if (Math.random() < 0.05) takeDamage(CONFIG.ENEMY.attackDamage * 0.1);
        }
    ...
    setState(newState) {
        ...
        this.state = newState;
        if (newState === 'CHASE' && typeof playSound === 'function') playSound('scare');
    }
...
function updateHorrorEffects(dist, camera) {
    ...
    if(vig) vig.style.boxShadow = `inset 0 0 ${100 + intensity*100}px ${intensity*50}px black`;
    ...
    if (typeof audioCtx !== 'undefined' && audioCtx && dist < 10 && Math.random() < 0.02) {
        playSound('heartbeat');
    }
}
